...
import { createPickupPoint, updatePickupPoint, PickupPoint } from '../../api/fleet';
...
export function PickupPointModal({ routeId, pointToEdit, onClose, onSaved }: PickupPointModalProps) {
  ...
  useEffect(() => {
    if (pointToEdit) {
      ...
      // Geolocalização aproximada do usuário como fallback inicial (São Paulo por padrão)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim()) {
      setError('O nome do ponto de embarque é obrigatório.');
      return;
    }
    ...
    try {
      if (pointToEdit) {
        await updatePickupPoint(routeId, pointToEdit.id, { name, lat, lng });
      } else {
        await createPickupPoint(routeId, { name, lat, lng });
      }
      onSaved();
    } catch (err: any) {
      setError(err.response?.data?.message || 'Erro ao salvar o ponto de embarque.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white rounded-[18px] w-full max-w-2xl flex flex-col max-h-[90vh] overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between p-6 border-b border-[#C3C6D7]/20 flex-shrink-0">
          <h3 className="text-lg font-bold text-[#131B2E]">
            {pointToEdit ? 'Editar Ponto de Embarque' : 'Adicionar Ponto de Embarque'}
          </h3>
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 flex flex-col gap-5">
          ...
          <Input
            id="point-name"
            label="Nome do Ponto de Embarque"
            placeholder="Ex: Parada da Igreja Matriz"
            value={name}
